...
[![Download DMG](https://img.shields.io/badge/Download-macOS%20DMG-black?logo=apple)](https://github.com/xx239/X-JSON/releases/latest/download/X-JSON-macOS.dmg)

## 下载

- macOS (DMG, 最新版): [X-JSON-macOS.dmg](https://github.com/xx239/X-JSON/releases/latest/download/X-JSON-macOS.dmg)
- 历史版本: [GitHub Releases](https://github.com/xx239/X-JSON/releases)
...
- 错误提示包含原因、行列和上下文片段。
- 非法 JSON 不会导致崩溃。
- 支持一类常见容错：`{79871452168:16999}` 这类未加引号 key 会尝试自动规范化后解析。
...
- Launch with default window
- Appearance theme
- Editor font / font size
- Background opacity
...
- Auto parse clipboard JSON
- On clipboard JSON detected: Create new tab / Overwrite current tab
...
- 当前仓库已忽略构建产物与用户态缓存文件（如 `.build/`、`.swiftpm/`、`*.xcuserstate`）。
